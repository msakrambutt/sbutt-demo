  ...
  try {
    if (process.env.SECRET_KEY) {
      ...
      if (body.clientEmail && body.clientOldPwd && body.clientNewPwd) {
        ...
        const enteredPassword = body.clientOldPwd;
        ...
        if (typeof body.clientOldPwd === "string") {
          ...
          if (!passwordCompare) {
            return new NextResponse(
              JSON.stringify({ message: "Your old password is Incorrect!", status: 400 })
            );
          }
          ...
          return new NextResponse(JSON.stringify({ message:"User password has been changed sucessfully! "}));
